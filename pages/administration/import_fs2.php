...
if (isset($_POST['step'])) {
  ...
  if ($_POST['step'] == 'save') {
    ...
    foreach($scores as $score) {
        ...
        echo '<td style="font-size:0.8em" class="run">'.$run.'</td>';
...
$(function(){
  ...
  $('#send').click(function() {
    ...
    $('tr.correct').each(function(i, elem) {
      ...
      run.push($(this).find('.run').text());
      numbers.push($(this).find('.number').text());
      teams.push($(this).find('.team').data('id'));
    });

    $.post('?page=administration&admin=import_fs2',{
      'step': 'save',
      ...
      'numbers[]': numbers
        ...
        if (data.indexOf('SUCCESS ---- SUCCESS') > 0) {
            window.location = '?page=administration&admin=import_fs2'
        } else {
            alert(data);
        }
    });
...
<div class="container">
  <form method="post" action="">
    ...
    <input type="text" name="spalten" value="team,time,run" style="width:500px"/><br/>
