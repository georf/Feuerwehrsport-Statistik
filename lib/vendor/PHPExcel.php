...
function setExcelHead($worksheet, $cell, $value) {
    $worksheet->setCellValue($cell, $value);
    setExcelBold($worksheet, $cell);
    setExcelBorder($worksheet, $cell);
    setExcelHCenter($worksheet, $cell);
}
function setExcelBackground($worksheet, $cell, $color) {
...
function setExcelBold($worksheet, $cell, $size = false) {
    $worksheet->getStyle($cell)->applyFromArray(array(
       'font' => array(
            'bold' => true,
        ),
...
function setExcelTime($worksheet, $cell, $time) {
    if ($time == PHP_INT_MAX || $time == 'D' || $time == 99999999) {
        $worksheet->setCellValue($cell, 'D');
    } else {
        $worksheet->setCellValue($cell, time2string($time));
        $worksheet->getStyle($cell)->getNumberFormat()->setFormatCode('#.00');
    }
    setExcelHCenter($worksheet, $cell);
}
function setExcelBorder($worksheet, $cell) {
    $worksheet->getStyle($cell)->applyFromArray(array(
        'borders' => array(
            'allborders' => array(
...
function setExcelHCenter($worksheet, $cell) {
    $worksheet->getStyle($cell)->applyFromArray(array(
        'alignment' => array(
...
function setExcelVCenter($worksheet, $cell) {
    $worksheet->getStyle($cell)->applyFromArray(array(
        'alignment' => array(
